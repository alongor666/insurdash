...
- **逻辑**: 选择后，直接调用 `setPeriod` 或 `setComparePeriod` 更新全局状态，立即触发整个仪表盘的数据重算。在“年累计”和“当周”分析模式下，“对比周期”选择器会被禁用，因为对比基准是隐含的上一周期。

### 3.2. 分析模式切换
- **组件**: 一个三选项的 `RadioGroup` 单选按钮组，是仪表盘数据分析维度的总开关。
- **逻辑**: 在三种分析模式之间切换，会立即触发整个仪表盘所有组件（KPI卡片、图表、数据表）的数据重算。
    - **年累计 (YTD)**:
        - **数据口径**: 所有数据基于**年累计值(YTD)**。
        - **对比逻辑**: 对比 `当前周期` 与 `上一连续周期` 的累计值。
    - **当周 (PoP)**:
        - **数据口径**: 所有数据基于**当周发生额(PoP)**。
        - **对比逻辑**: 对比 `当前周期` 与 `上一连续周期` 的当周发生额。
    - **对比 (Comparison)**:
        - **数据口径**: 所有数据基于**年累计值(YTD)**。
        - **对比逻辑**: 对比 `当前周期` 与用户**自由选择的对比周期**的累计值。
