...
* **2.3.3. 逻辑处理核心 (data-utils.ts)**  
  * 所有的数据转换和KPI计算逻辑都被严格地封装在 src/lib/data-utils.ts 文件中。  
  * **核心函数**: processSupabaseData 是数据处理的入口。它接收原始的 PeriodData 数组和用户的筛选条件（周期、业务线、分析模式），然后调用 calculateAllKpis 函数。  
  * **计算引擎**: calculateAllKpis 是应用的大脑。它根据分析模式（“累计”或“当周”）和选定的业务范围，对原始数据进行聚合、相减（PoP模式下），并最终计算出**全部16个核心KPI**的精确值。  
  * **输出**: processSupabaseData 的最终输出是一个 ProcessedData 对象，它包含了所有UI组件（KPI卡片、图表、数据表）渲染所需的、已经过完整计算和处理的最终数据。

这个结构确保了数据源的单一性和计算逻辑的一致性。任何关于KPI口径的修改，都只需要在calculateAllKpis一个地方进行。
...
* **3.2.3. 主内容区 (Main Content Area)**  
  ...
  * **B. 交互式图表区 (Tabs)**  
    ...
    * **五个标签页**:  
      ...
      3. **业务排名**: 包含一个KPI选择器，图表会根据所选指标对所有业务线进行自动排序。  
...
* **实现细节**:  
  ...
  2. **业务类型筛选 (Business Type Filter)**:  
     ...
     * **交互**:  
       * 点击按钮会弹出一个包含所有业务类型列表的浮层 (Popover)。  
       * 浮层内提供一个**搜索框**，可实时过滤业务类型列表。  
       * 提供\*\*“全选”、“反选”、“清空”\*\*三个快捷操作按钮。  
       * 用户可以通过勾选复选框来修改选择。  
       * 点击\*\*“取消”**按钮会关闭浮层并放弃本次修改；点击**“确认”\*\*按钮会应用本次修改，并触发仪表盘数据重算。  
...
* **实现细节**:  
  ...
  2. **卡片内容 (KpiCard.tsx)**:  
     ...
     * **对比分析**: 在数值下方，展示与“对比周期”的差异，由 getComparisonText 函数生成。  
       * **颜色**: 根据 KPI\_CONFIG 中定义的 positiveChangeIs 属性，如果指标向好（如利润增长、成本下降），文本为**绿色**；如果向差，则为**红色**。  
       * **内容**: 同时包含绝对值变化和百分比变化，例如 \+1.5 p.p. (+10.5%)。
...
* **实现细节**:
  ...
  * #### **4.3 业务排名 (Ranking Chart)**

    * **组件**: RankingChart.tsx  
    * **目标**: 通过可排序的水平条形图，对所有业务线按选定的核心指标进行排名。  
    * **交互**: 顶部提供一个 KpiSelector 允许用户选择任意一个KPI作为排名依据。图表数据会根据所选KPI的值**从高到低**自动排序。  
    * **数据洞察**:  
...
### **F-REQ-05: 详细数据表 (Detailed Data Table)**

* **目标**: 在仪表盘底部提供一个默认折叠的、支持横向滚动的详细数据表格，作为所有图表的数据基础和最终验证工具。  
* **实现细节**:  
  1. **交互**:  
     * **可折叠**: 使用 Collapsible 组件，默认收起。用户可点击展开或收起，在概览和细节之间自由切换。  
     * **横向滚动**: 使用 ScrollArea 组件，当表格内容超出屏幕宽度时，自动出现横向滚动条。  
     * **表头/首列固定**: 为提升体验，表格的表头 (\<thead\>) 和第一列（业务类型）在滚动时会保持固定。  
     * **表头排序**: 所有16个指标列的表头均支持点击排序。use-dashboard-state 维护排序状态（sortConfig），点击时会切换升序/降序，并重新渲染表格。当前排序列的表头旁会显示一个醒目的排序箭头。  
  2. **数据与样式**:  
     * **内容**: 完整呈现当前筛选条件下，所有业务线的全部16个核心指标的精确数值。  
     * **动态着色**: 特定几列（如变动成本率、边际贡献率等，由 COLORED\_KPIS 集合定义）的单元格数值会根据该行的variable\_cost\_ratio进行动态着色，高风险指标显示为红色/紫色，健康指标显示为绿色。
...
* **实现细节**:  
  1. **动态风险色 (Business Line Color)**:  
     ...
     * **规则**: 系统会根据每个业务线在**当前筛选条件**下的\*\*variable\_cost\_ratio (VCR)\*\* 的值，动态计算出一个从\*\*健康（深绿）**到**高风险（深红/紫色）\*\*的渐变色。  
     * **应用范围**: 这个颜色被统一应用于所有图表（气泡图、排名图、帕累托图等）和数据表中，用于着色代表该业务线的图形元素（如条形、气泡）或文本，帮助用户在不同视图中快速识别和追踪同一业务线的表现及其风险状况。  
...
本字典定义了应用中所有核心指标的计算口径、业务含义与派生关系，确保全公司对数据理解的一致性。计算逻辑基于 src/lib/data-utils.ts 中的 calculateAllKpis 函数。
